...
const SignIn = () => {
  ...
  // Centralized helper to extract Clerk's nested error array messages cleanly
  const handleClerkError = (error: any) => {
    console.error("Clerk raw error:", JSON.stringify(error, null, 2));

    // Pull clean text from Clerk's specific API error list array structure
    const clerkErrorMessage =
      error?.errors && error.errors.length > 0
        ? error.errors[0].longMessage || error.errors[0].message
        : error?.message || "An unexpected error occurred. Please try again.";

    // 1. Send clean text to PostHog analytics
    posthog.capture("user_sign_in_failed", {
      error_message: clerkErrorMessage,
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Handle structural error returns if present on the returned payload
      if (response?.error) {
        handleClerkError(response.error);
        return;
      }
      ...
      // Handles unhandled promise rejections / network exceptions
  ...
  const handleVerify = async () => {
    try {
      const response = await signIn.mfa.verifyEmailCode({ code });
